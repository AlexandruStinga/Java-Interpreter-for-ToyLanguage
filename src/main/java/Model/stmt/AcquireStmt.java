        ...
        if(symTable.isDefined(var))
        {
            ...
            if(varValue.getType().equals(new IntType()))
            {
                ...
                if(semaphoreTable.contains(foundIndex))
                {
                    ...
                    Integer NL = pair.getSecond().size();
                    Integer N1 = pair.getFirst();
                    if(N1 > NL) {
                        Integer identifier = state.getStateID();
                        if (pair.getSecond().contains(identifier))
                        {

                        }
                        else pair.getSecond().add(identifier);
                    }
                    else stack.push(new AcquireStmt(var));
                }
                else throw new MyException("semaphore table doesnt contain var");
            }
            else throw new MyException("var is not an int (acquire)");
        }
        else throw new MyException("var is not in symtable (acquire)");
